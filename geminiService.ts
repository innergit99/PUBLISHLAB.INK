...
This chapter is awaiting AI generation. Click "Generate" to create this chapter.
...
AVOID: Any imagery that could interfere with barcode scanning`;
  ...
  async generateImageForModule(prompt: string, module: 'POD' | 'KDP' | 'MOCKUP' | 'KDP_INTERIOR' = 'POD', options: { forceEngine?: 'POLLINATIONS' | 'HF_ZERO_GPU' | 'DEEPAI', aspectRatio?: string, negativePrompt?: string, colorMode?: 'Color' | 'B&W', author?: string, genre?: string } = {}): Promise<string> {
    ...
    try {
      ...
      // Note: hfBackend.generateImage handles the interaction
      const imageUrl = await hfBackend.generateImage(finalPrompt, options.negativePrompt, width, height);
      return imageUrl;
    } catch (e) {
      console.warn("❌ HF Backend Image Gen failed, falling back...", e);
    }

    // --- TIER 2: POLLINATIONS (Deprecated/Backup) ---
    // Keeping as deep backup only if HF fails completely
    // CRITICAL FIX: Skip Pollinations for KDP/KDP_INTERIOR (returns 403 during PDF export or low quality)
    if (module !== 'KDP' && module !== 'KDP_INTERIOR') {
      try {
        console.log("⚠️ Attempting Pollinations as backup (POD only)...");
        return await this.generateWithPollinations(finalPrompt, width, height, options.negativePrompt || "", module === 'POD');
      } catch (e) {
        console.warn("Pollinations backup failed.", e);
      }
    }

    // --- TIER 2.5: ANTIGRAVITY IDE IMAGE GENERATION ---
    // Use Antigravity IDE's built-in Nano Banana Pro (Gemini 3 Pro Image / Gemini 2.5 Image)
    // Specialized for high-fidelity image generation, text rendering, and edit-to-edit consistency
    // Images are generated as "Artifacts" in the IDE workflow
    try {
      console.log("🤖 [IDE Resilience] Using Antigravity IDE's Nano Banana Pro (Gemini 3 Pro Image)...");

      // Use IDE's generate_image tool (MCP integration)
      // The IDE exposes this via the generate_image tool
      const idePrompt = enhancedPrompt.length > 1000 ? enhancedPrompt.substring(0, 1000) : enhancedPrompt;

      // Call the IDE's image generation tool
      // This will use Gemini Nano Banana Pro model
      const ideImageUrl = await this.generateWithIDETool(idePrompt, width, height);
    ...
    // --- TIER 3: CANVAS FALLBACK (Always Works) ---
    if (module === 'KDP' || module === 'KDP_INTERIOR') {
      console.log('🎨 [Industrial Resilience] Using advanced canvas-based cover generator');
      const { coverGenerator } = await import('./coverGenerator');

      const titleMatch = prompt.match(/"([^"]+)"/);
      const title = titleMatch ? titleMatch[1] : 'Untitled';

      if (module === 'KDP_INTERIOR' || prompt.toLowerCase().includes('chapter')) {
        return await coverGenerator.generateInteriorImage(
          prompt,
          title.length < 3 ? "Chapter Art" : title,
      ...
      return await coverGenerator.generateCover({
        title: title,
        author: options.author || 'Artisan author',
    ...
    // --- TIER 4: UNIVERSAL CANVAS FALLBACK (Elite Resilience) ---
    console.warn("⚠️ All AI engines failed. Resynchronizing via Industrial Canvas...");
    const { coverGenerator } = await import('./coverGenerator');
    return await coverGenerator.generateGenericPlaceholder(prompt, width, height);
  }
